...
selected_opt = st.sidebar.radio(label="Choose the DB to connect", options=radio_opt)
...
if not db_uri:
    st.info("Please select the database to connect")
...
def configure_db(
    ...
    if db_uri == LOCALDB:
        ...
        if not (mysql_host and mysql_user and mysql_password and mysql_db):
            st.error("Please provide the MySQL details")
            st.stop()
        return SQLDatabase(
            create_engine(
